...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel private(rand_x, rand_y) reduction(+:circle_points)
    {
        int tid = omp_get_thread_num();
        unsigned int seed = (unsigned int)time(NULL) * (tid + 1); // Create unique seed for each thread

        #pragma omp for
        for (i = 0; i < N; i++) {
            rand_x = (double)rand_r(&seed) / RAND_MAX; // Use rand_r with unique seed for each thread
