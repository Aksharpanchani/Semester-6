...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel for num_threads(P) private(i, j) shared(B, B_transpose) schedule(static) collapse(2)
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
...
#pragma omp parallel for num_threads(P) private(i, j, k) shared(A, B_transpose, C) schedule(static) collapse(2)
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
